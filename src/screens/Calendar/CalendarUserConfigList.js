...
const CalendarUserConfigList = () => {
  const state = useSelector(state => state)
  const calendarState = state.calendar?.data
  // const failure = state.calendar?.failure
  ...
  useEffect(() => {
    const getData = async () => { 
      setRequestApi(true)
      http.get(`/calendarm/users?PageSize=10${currentPage ? `&page=${currentPage}` : ''}${search ? `&search=${search}` : ''}`)
        .then((data) => {
          setRequestApi(false)
  ...
  }, [currentPage, search, calendarState]);
          ...
          <>
            <View style={{ marginVertical: 5, padding: 10, borderWidth: 1, borderColor: "#ccc", borderRadius: 5, backgroundColor: "#fff" }} key={index}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: 'center' }} >
                ...
                <View >
                  <ModalUserSupervisor dataUser={data} type={"Calendar"} />
                </View>
              </View>
            </View>
          </>
...
export default CalendarUserConfigList
