...
const GPSUserConfigList = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      setRequestApi(true)
      http.get(`/gps/users?PageSize=10${currentPage ? `&page=${currentPage}` : ''}${search ? `&search=${search}` : ''}`)
        .then((data) => {
          setRequestApi(false)
    ...
    getData()
  }, [currentPage, search, gpsTrackState]);
          ...
          <Fragment key={data.userId}>
            <View style={{ marginVertical: 5, padding: 10, borderWidth: 1, borderColor: "#ccc", borderRadius: 5, backgroundColor: "#fff" }} key={index}>
              <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: 'center' }} >
                ...
                <View >
                  <ModalUserSupervisor dataUser={data} type={"Gps"} />
                </View>
              </View>
            </View>
          </Fragment>
...
export default GPSUserConfigList
